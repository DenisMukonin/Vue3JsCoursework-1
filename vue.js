...
const App = {
  ...
  computed: {
    ...
    lastStep(event) {
      ...
      //Не понял, как тут определить, находимся ли мы на последнем шаге, а точнее как сделать это через computed
    },
    currentStep(event) {
      // this.activeIndex
      //Определяю текущий шаг в других методах, так же не совсем разобрался, как сделать это через computed
    }
